...
# Williams %R
# -------------------------
def test_williams_r_not_ready_until_period() -> None:
    wr = WilliamsR(period=3)

    assert wr.update(c(1, 2, 0, 1)) is None
    assert wr.ready() is False

    assert wr.update(c(1, 3, 0, 2)) is None
    assert wr.ready() is False

    v = wr.update(c(2, 4, 1, 3))
...
def test_williams_r_flat_range_returns_minus_50() -> None:
    wr = WilliamsR(period=3)

    # All highs == lows == 10 -> HH==LL -> defined as -50.0
    assert wr.update(c(10, 10, 10, 10)) is None
    assert wr.update(c(10, 10, 10, 10)) is None
    v = wr.update(c(10, 10, 10, 10))
    assert v == pytest.approx(-50.0)


def test_williams_r_reset() -> None:
    wr = WilliamsR(period=2)
    assert wr.update(c(1, 2, 0, 1)) is None
    v = wr.update(c(1, 2, 0, 1))
...
# MFI
# -------------------------
def test_mfi_not_ready_until_period_classified_flows() -> None:
    ...
    assert mfi.update(c(1, 2, 0, 1, volume=10)) is None
    assert mfi.ready() is False

    # 2nd candle -> 1 classified flow, still not ready
    assert mfi.update(c(1, 3, 0, 2, volume=10)) is None
    assert mfi.ready() is False

    # 3rd candle -> 2 classified flows, still not ready
    assert mfi.update(c(2, 4, 1, 3, volume=10)) is None
    assert mfi.ready() is False

    # 4th candle -> 3 classified flows => ready
    v = mfi.update(c(3, 5, 2, 4, volume=10))
    assert mfi.ready() is True
    assert isinstance(v, float)


def test_mfi_known_value_simple_sequence() -> None:
    """
    ...
    assert mfi.update(c(1, 1, 1, 1, volume=1)) is None  # tp=1
    assert mfi.update(c(2, 2, 2, 2, volume=1)) is None  # +2
    assert mfi.update(c(3, 3, 3, 3, volume=1)) is None  # +3
    v = mfi.update(c(2, 2, 2, 2, volume=1))             # -2 => ready
    assert v == pytest.approx(100.0 - (100.0 / (1.0 + (5.0 / 2.0))), rel=1e-12)


def test_mfi_volume_fallback_to_tick_count() -> None:
    # If volume is 0, MFI uses tick_count as volume surrogate.
    mfi = MFI(period=2)

    # First candle: no prev TP
    assert mfi.update(c(1, 1, 1, 1, volume=0, tick_count=10)) is None
    # Second candle: classify one flow (+)
    assert mfi.update(c(2, 2, 2, 2, volume=0, tick_count=10)) is None
    # Third candle: classify second flow (+) => ready => neg=0 pos>0 => 100
    v = mfi.update(c(3, 3, 3, 3, volume=0, tick_count=10))
    assert v == pytest.approx(100.0)


def test_mfi_neutral_when_no_flow() -> None:
    # With your patch: if pos==0 and neg==0 => return 50.0
    mfi = MFI(period=2)

    # constant typical_price: no positive/negative flows
    assert mfi.update(c(1, 1, 1, 1, volume=1)) is None
    assert mfi.update(c(1, 1, 1, 1, volume=1)) is None
    v = mfi.update(c(1, 1, 1, 1, volume=1))
    assert v == pytest.approx(50.0)


def test_mfi_reset() -> None:
    mfi = MFI(period=2)
    assert mfi.update(c(1, 1, 1, 1, volume=1)) is None
    assert mfi.update(c(2, 2, 2, 2, volume=1)) is None
    v = mfi.update(c(3, 3, 3, 3, volume=1))
    assert v is not None
    assert mfi.ready()
...
# MACD
# -------------------------
def test_macd_returns_stable_dict_shape_during_warmup() -> None:
    macd = MACD(fast=3, slow=5, signal=2)

    out1 = macd.update(c(1, 1, 1, 1))
    ...
    out2 = macd.update(c(2, 2, 2, 2))
    assert set(out2.keys()) == {"macd", "signal", "histogram"}


def test_macd_strict_ready_only_when_signal_available() -> None:
    # With SMA seeding:
    ...
    for price in [1, 2, 3, 4, 5, 6, 7, 8]:
        outputs.append(macd.update(c(price, price, price, price)))
...
def test_macd_reset() -> None:
    macd = MACD(fast=3, slow=5, signal=2)

    for price in [1, 2, 3, 4, 5, 6, 7]:
        macd.update(c(price, price, price, price))

    assert macd.ready() is True

    macd.reset()
    assert macd.ready() is False
    out = macd.update(c(1, 1, 1, 1))
