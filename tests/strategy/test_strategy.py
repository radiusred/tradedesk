...
class TestBaseStrategy:
    ...
    async def test_run_without_lightstreamer(self):
        ...
        mock_client.get_market_snapshot = AsyncMock(
            return_value={"snapshot": {"bid": 1.2345, "offer": 1.2347}}
        )
        ...
        class TestStrategy(BaseStrategy):
            ...
            async def on_price_update(self, market_data):
                self.updates.append(
                    (market_data.instrument, market_data.bid, market_data.offer)
                )
    ...
    async def test_polling_mode(self):
        ...
        mock_client.get_market_snapshot = AsyncMock(
            side_effect=[
                ...
                {
                    "snapshot": {"bid": 1.1001, "offer": 1.1003}
                },  # Second EURUSD (changed)
    ...
    async def test_streaming_mode_setup(self, mock_lightstreamer):
        ...
        mock_client.get_streamer = lambda: Lightstreamer(mock_client)
        ...
        import lightstreamer.client as ls_client_module  # type: ignore[import]

        ls_client_module.LightstreamerClient = lambda *args, **kwargs: mock_ls_info[
            "client"
        ]
        ls_client_module.Subscription = lambda *args, **kwargs: mock_ls_info[
            "subscription"
        ]
