...
class TestRunner:
            ...
            for actual_call in mock_info.call_args_list:
                try:
                    ...
                    if (
                        ...
                        and actual_call[0][1] == "TestStrategy"
                        and actual_call[0][2] == 1
                    ):
                        found = True
                        break
    ...
    async def test_run_strategies_async_multiple(self):
        ...
        mock_strategy1.subscriptions = [
            SimpleNamespace(instrument="CS.D.EURUSD.CFD.IP")
        ]
        mock_strategy1.__class__.__name__ = "Strategy1"
        mock_strategy1.run = AsyncMock()

        mock_strategy2 = MagicMock()
        mock_strategy2.subscriptions = [
            SimpleNamespace(instrument="CS.D.GBPUSD.CFD.IP")
        ]
