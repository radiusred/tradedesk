...
class TestIGClient:
    ...
    async def test_initialization_live(self):
        with patch("tradedesk.execution.ig.client.settings") as mock_settings:
    ...
    @pytest.mark.asyncio
    async def test_place_market_order_defaults_include_expiry_tif_gbp_netting(
        ...
        with patch("aiohttp.ClientSession", return_value=mock_aiohttp_session):
            ...
            res = await client.place_market_order(
                instrument="CS.D.EURUSD.TODAY.IP", direction="BUY", size=1.0
            )
    ...
    @pytest.mark.asyncio
    async def test_place_market_order_sets_dfb_expiry_on_spreadbet_account(
        self, mock_aiohttp_session
    ):
        mock_response = MagicMock()
        mock_response.status = 200
    ...
    async def test_get_instrument_metadata_caches_results(self, mock_aiohttp_session):
        ...
        with patch("aiohttp.ClientSession", return_value=mock_aiohttp_session):
            ...
            metadata3 = await client.get_instrument_metadata(
                "CS.D.EURUSD.TODAY.IP", force_refresh=True
            )
    ...
    async def test_quantise_size_with_three_decimal_places(self, mock_aiohttp_session):
        ...
        with patch("aiohttp.ClientSession", return_value=mock_aiohttp_session):
            ...
            client._instrument_metadata["TEST.CRYPTO"] = {
                "dealingRules": {"minDealSize": {"value": 0.001}}
            }
    ...
    async def test_quantise_size_very_large_numbers(self, mock_aiohttp_session):
        ...
        with patch("aiohttp.ClientSession", return_value=mock_aiohttp_session):
            ...
            client._instrument_metadata["IX.D.DOW.LARGE"] = {
                "dealingRules": {"minDealSize": {"value": 10}}
            }
    ...
    async def test_place_market_order_confirmed_waits_for_confirmation(self, mock_aiohttp_session):
        ...
        confirm_response.json = AsyncMock(
            return_value={"dealStatus": "ACCEPTED", "dealId": "DEAL123"}
        )
    ...
    @pytest.mark.asyncio
    async def test_place_market_order_confirmed_raises_without_deal_reference(
        self, mock_aiohttp_session
    ):
    ...
    async def test_place_market_order_confirmed_raises_rejected_status(self, mock_aiohttp_session):
        ...
        confirm_response.status = 200
        confirm_response.json = AsyncMock(
            return_value={"dealStatus": "REJECTED", "dealId": "DEAL123"}
        )
        confirm_response.__aenter__ = AsyncMock(return_value=confirm_response)
        confirm_response.__aexit__ = AsyncMock(return_value=None)
        ...
        with patch("aiohttp.ClientSession", return_value=mock_aiohttp_session):
            ...
            with pytest.raises(DealRejectedException, match="Deal rejected"):
                await client.place_market_order_confirmed(
                    instrument="CS.D.EURUSD.TODAY.IP", direction="BUY", size=1.0
                )
