...
class IGClient:
    ...
    def __init__(self):
        ...
        # Persistent session – holds headers & cookies across calls
    ...
    def _authenticate(self) -> None:
        ...
        if resp.status_code != 200:
            ...
            raise RuntimeError(
                f"IG authentication failed – HTTP {resp.status_code}. "
        ...
        if oauth_token and oauth_token.get("access_token"):
            self._store_oauth_token(oauth_token, json_body.get("accountId", ""), json_body.get("clientId", ""))
        ...
        # --------------------------------------------------------------
        def _find_token(name: str, json_key: str) -> str | None:
            """
            ...
            # b) Cookie jar (some older responses)
            token = resp.cookies.get(name)
            if token:
                log.info("%s obtained from response cookie.", name)
                return token
        ...
        # --------------------------------------------------------------
        self.session.headers.update({"CST": cst, "X-SECURITY-TOKEN": x_sec})
        self.ls_cst = cst
        self.ls_xst = x_sec
    ...
    # OAuth token management
    # ------------------------------------------------------------------
    def _store_oauth_token(self, oauth_token: Dict[str, Any], account_id: str, client_id: str) -> None:
    ...
    def _request(self, method: str, path: str, **kwargs) -> Dict[str, Any]:
        ...
        if resp.status_code in (401, 403):
            ...
            log.warning("Auth failed (HTTP %s) – attempting re-authentication", resp.status_code)
            self._authenticate()
            resp = self.session.request(method, url, **kwargs)
    ...
    def get_price_ticks(self, epic: str) -> Dict[str, Any]:
        """Convenient shortcut to the "prices" endpoint – useful for polling."""
        return self._request("GET", f"/prices/{epic}")
