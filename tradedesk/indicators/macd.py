...
class MACD(Indicator):
    ...
    def update(self, candle: Candle) -> dict[str, float | None]:
        ...
        if self.fast_ema is not None:
            self.fast_ema = (close * self.fast_multiplier) + (self.fast_ema * (1 - self.fast_multiplier))
        
        if self.slow_ema is not None:
            self.slow_ema = (close * self.slow_multiplier) + (self.slow_ema * (1 - self.slow_multiplier))
        ...
        if self.signal_ema is not None:
            self.signal_ema = (macd_line * self.signal_multiplier) + (self.signal_ema * (1 - self.signal_multiplier))
